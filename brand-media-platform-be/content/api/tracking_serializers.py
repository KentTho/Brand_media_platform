...
class UserBehaviorSerializer(serializers.Serializer):
    ...
    scroll_depth = serializers.IntegerField(min_value=0, max_value=100, required=False, default=0)  # Why: Bounds to reduce invalid data bug
    time_spent = serializers.IntegerField(min_value=0, required=False, default=0)

    meta_data = serializers.JSONField(required=False, default=dict)  # Why: Flexible for AI extras (e.g., {'highlight_text': '...'})

    def validate_article(self, value):
        # Why: Ensure published article exists; defensive to prevent logging junk data (reduce bug)
        try:
            return Article.objects.get(slug=value, status="published", deleted_at__isnull=True)
