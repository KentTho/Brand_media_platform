...
class Product(TranslatableModel):
    translations = TranslatedFields(
        ...
        specifications=models.JSONField(default=dict, blank=True)  # Lưu size, màu dạng JSON
    )

    slug = models.SlugField(unique=True, db_index=True)
    #sku = models.CharField(max_length=50, unique=True, blank=True, null=True)  # Mã kho
    material = models.ForeignKey(Material, on_delete=models.PROTECT, related_name='products')
    ...
    related_articles = models.ManyToManyField('content.Article', blank=True, related_name='related_products')
