...
class CustomUserAdmin(BaseUserAdmin):
    ...
    search_fields = ('email', 'full_name')  # Fix: Use 'full_name' (non-translated field); remove 'translations__' to avoid field not found bug
    ordering = ('email',)

    fieldsets = (
        (None, {'fields': ('email', 'password')}),
        ('Personal info (Translatable)', {'fields': ('full_name', 'bio', 'avatar')}),  # Group full_name here even if not translated
    ...
    filter_horizontal = ('groups', 'user_permissions')  # Easier management for many-to-many fields
